...
public class TwoDimensionCodeUtil {
	 ...
	 *            logo头像存放路径(含文件名)
	 */
	private static void middleImage(String imagePath, String logoPath) throws IOException {
	 ...
	 *            logo头像存放路径(含文件名)
	 */
	private static void outlineImage(String imagePath, String logoPath) throws IOException {
		BufferedImage image = ImageIO.read(new File(imagePath));
		int logoWidth = image.getWidth() / 5; // 设置logo图片宽度为二维码图片的五分之一
		int logoHeight = image.getHeight() / 5; // 设置logo图片高度为二维码图片的五分之一
	...
	public static boolean encodeQRCodeImage(String content, String charset, String imagePath, int width, int height,
		...
		try {
			content = new String(content.getBytes(charset == null ? "UTF-8" : charset), "ISO-8859-1");
		...
		try {
			MatrixToImageWriter.writeToFile(bitMatrix, imagePath.substring(imagePath.lastIndexOf(".") + 1),
		...
		if (null != logoPath) {
			...
			try {
				...
				// middleImage(imagePath, logoPath);
				outlineImage(imagePath, logoPath);
				return true;
			} catch (IOException e) {
	...
	public static void main(String[] args) {
		String content = "CVS日常操作";
		TwoDimensionCodeUtil.encodeQRCodeImage(content, "GBK", "D:\\TwoDimensionCode.png", 200, 200,
				"D:\\anjianlog.png");
		System.out.println(TwoDimensionCodeUtil.decodeQRCodeImage("D:\\TwoDimensionCode.png", "GBK"));
	}
}