...
public class Dom4jUtils {
     ...
     * 将xml文件格式化输出到目标文件
     *
     * @param srcFile    xml源文件
     * @param targetFile 格式化输出xml文件
     * @throws IOException
     * @throws DocumentException
     */
     ...
     * @return
     */
    public static Document createXML(NodeVo vo) {
    ...
    public static void addChildsToNode(Element element, List<NodeVo> childNodes) {
        if (Objects.isNull(element) || Objects.isNull(childNodes)) {
            return;
        }
        ...
        for (NodeVo nodeVo : childNodes) {
            ...
            if (Objects.nonNull(nodeVo.getText())) {
                // 节点文本内容
    ...
    public static NodeVo recursiveDoc(Document doc) {
        ...
        while (nodeIter.hasNext()) {
            ...
            if (Objects.isNull(node) || (node.getNodeType() != Node.COMMENT_NODE && node.getNodeType() != Node.ELEMENT_NODE)) {
                continue;
            }
