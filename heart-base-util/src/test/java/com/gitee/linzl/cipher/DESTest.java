...
public class DESTest {
	...
	private void excuteCBC(IAlgorithm algorithm) throws Exception {
		...
		SymmetricCipherBuilder cipherBuilder = new SymmetricCipherBuilder(algorithm, rawSecretKey);
		byte[] encryptData = cipherBuilder.ivParameterSpec(ivParameterSpec).encrypt(text.getBytes());
		BasePrint.printEncryptData(encryptData);

		cipherBuilder.ivParameterSpec(ivParameterSpec);
		byte[] decryptData = cipherBuilder.ivParameterSpec(ivParameterSpec).decrypt(encryptData);
