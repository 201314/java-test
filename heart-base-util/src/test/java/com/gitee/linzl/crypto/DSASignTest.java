...
public class DSASignTest {
	...
	private void rsaSignRandom(SignatureAlgorithms algorithm) throws Exception {
		...
		byte[] encryptData = SignatureCipherUtil.sign(text.getBytes(), privateKey, algorithm);
		System.out.println("加密：" + encryptData);

		PublicKey publicKey = keyPair.getPublic();
		System.out.println("公钥长度：" + Hex.toHexString(publicKey.getEncoded()));
		boolean verifyResult = SignatureCipherUtil.verifySign(text.getBytes(), publicKey, encryptData, algorithm);
