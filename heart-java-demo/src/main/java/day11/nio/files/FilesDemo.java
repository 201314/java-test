...
public class FilesDemo {
	...
	public static void newByte() throws IOException {
		Path path1 = Paths.get("D:\\trawe_store\\第2个接口.txt");
		SeekableByteChannel reader = Files.newByteChannel(path1);

		Path path2 = Paths.get("D:\\trawe_store\\第2个接口---newByte.txt");
		SeekableByteChannel writer = Files.newByteChannel(path2, StandardOpenOption.CREATE, StandardOpenOption.WRITE,
	...
	public static void newDirectory() throws IOException {
		...
		DirectoryStream<Path> reader = Files.newDirectoryStream(path1);
		reader.forEach((path) -> {
			System.out.println(path.toFile());
		});
	}
	...
	public static void newStream() throws IOException {
		Path path1 = Paths.get("D:\\trawe_store\\第2个接口.txt");
		InputStream reader = Files.newInputStream(path1);
